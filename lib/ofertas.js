...
/**
 * Um item so e vendavel quando tem id, product_hash, titulo e preco > 0.
 *
 * `offer_hash` e exigido apenas do front: ele vai no topo do payload da
 * transacao. Os bumps entram so no `cart`, que identifica por product_hash.
 */
function configurado(item) {
  if (!item || !item.id || !item.product_hash || !item.title || !(item.price_cents > 0)) return false;
  if (item.role === 'front' && !item.offer_hash) return false;
  return true;
}
...
function bumpsParaCliente() {
  return BUMPS.filter(configurado).map(b => ({
    ...
    amountDe: b.price_from_cents || null
  }));
}
